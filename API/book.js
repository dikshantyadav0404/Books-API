...
Router.put("/update/:isbn", async(request, response) => {
    const updatedBook = await BookModel.findOneAndUpdate({ ISBN: request.params.isbn }, { title: request.body.title }, { new: true });
...
Router.put("/updateAuthour/:isbn", async(request, response) => {
    const updatedBook = await BookModel.findOneAndUpdate({ ISBN: request.params.isbn }, { $addToSet: { authors: request.body.newAuthor } }, { new: true });

    const updatedAuthor = await AuthorModel.findOneAndUpdate({ id: request.body.newAuthor }, { $addToSet: { books: request.params.isbn } }, { new: true });
...
Router.delete("/deleteBook/:BookID", async(request, response) => {
    const updatedBook = await BookModel.findOneAndDelete({ ISBN: request.params.BookID });
...
Router.delete("/deleteAuthor/:BookID/:authorID", async(request, response) => {
    ...
    const updatedBook = await BookModel.findOneAndUpdate({ ISBN: isbn }, { $pull: { authors: author_ } }, { new: true });

    const updatedAuthor = await AuthorModel.findOneAndUpdate({ id: author_ }, { $pull: { books: isbn } }, { new: true });
